...
var React4, import_prop_types4, import_jsx_runtime5, import_jsx_runtime6, _excluded2, useUtilityClasses, defaultManager, ModalUnstyled, ModalUnstyled_default;
var init_ModalUnstyled = __esm({
  "node_modules/@mui/base/ModalUnstyled/ModalUnstyled.js"() {
    ...
    ModalUnstyled = React4.forwardRef(function ModalUnstyled2(props, forwardedRef) {
...
var React6, import_prop_types6, ReactDOM2, import_jsx_runtime8, import_jsx_runtime9, _excluded3, styles, TextareaAutosize, TextareaAutosize_default;
var init_TextareaAutosize = __esm({
  "node_modules/@mui/base/TextareaAutosize/TextareaAutosize.js"() {
    ...
    TextareaAutosize = React6.forwardRef(function TextareaAutosize2(props, ref) {
      const {
        onChange,
        maxRows,
        minRows = 1,
...
var React7, defaultFilterOptions, pageSize, defaultIsActiveElementInListbox;
...
var React8, import_prop_types7, import_jsx_runtime10, import_jsx_runtime11, _excluded4, useUtilityClasses2, BadgeUnstyled;
...
function useControllableReducer(internalReducer, externalReducer, props) {
  ...
  const combinedReducer = React17.useCallback((state, action) => {
  ...
  const [nextState, dispatch] = React17.useReducer(combinedReducer, initialState);
  const dispatchWithProps = React17.useCallback((action) => {
    dispatch(_extends({
      props: props.current
    }, action));
  }, [dispatch, props]);
  const previousState = React17.useRef(initialState);
  React17.useEffect(() => {
...
function useListbox(props) {
  ...
  const id = useId(idProp);
  const defaultIdGenerator = React20.useCallback((_, index) => `${id}-option-${index}`, [id]);
  ...
  const createHandleOptionClick = React20.useCallback((option, other) => (event) => {
    var _other$onClick;
  ...
  const createHandleOptionPointerOver = React20.useCallback((option, other) => (event) => {
  ...
  const getOptionState = React20.useCallback((option) => {
    let selected;
  ...
  const getOptionTabIndex = React20.useCallback((optionState) => {
    if (focusManagement === "activeDescendant") {
      return void 0;
    }
  ...
  const getOptionProps = React20.useCallback((option, otherHandlers = {}) => {
...
var React20, defaultOptionComparer, defaultIsOptionDisabled, defaultOptionStringifier;
...
function useSelectChangeNotifiers() {
  const messageBus = useMessageBus();
  const notifySelectionChanged = React29.useCallback((newValue) => {
    messageBus.publish(SELECTION_CHANGE_TOPIC, newValue);
  }, [messageBus]);
  const notifyHighlightChanged = React29.useCallback((newValue) => {
    messageBus.publish(HIGHLIGHT_CHANGE_TOPIC2, newValue);
  }, [messageBus]);
  const registerSelectionChangeHandler = React29.useCallback((handler) => {
    return messageBus.subscribe(SELECTION_CHANGE_TOPIC, handler);
  }, [messageBus]);
  const registerHighlightChangeHandler = React29.useCallback((handler) => {
...
var React29, SELECTION_CHANGE_TOPIC, HIGHLIGHT_CHANGE_TOPIC2;
...
function useSelect(props) {
  ...
  const optionsMap = React30.useMemo(() => {
  ...
  const ignoreEnterKeyUp = React30.useRef(false);
  const ignoreClick = React30.useRef(false);
  const [listboxFocusRequested, requestListboxFocus] = React30.useState(false);
  const focusListboxIfRequested = React30.useCallback(() => {
  ...
  const createHandleListboxItemClick = React30.useCallback((otherHandlers) => (event) => {
    var _otherHandlers$onClic2;
  ...
  const listboxReducer = React30.useCallback((state, action) => {
    const newState = defaultListboxReducer(state, action);
  ...
  const optionValues = React30.useMemo(() => options.map((o) => o.value), [options]);
  let useListboxParameters;
  const isOptionDisabled = React30.useCallback((valueToCheck) => {
  ...
  const getOptionProps = React30.useCallback((optionValue, otherHandlers = {}) => {
...
@mui/base/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-E2OQGWG4.js.map
